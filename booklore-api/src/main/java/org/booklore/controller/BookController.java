...
public class BookController {
    ...
    @Operation(summary = "Toggle comic flag", description = "Mark or unmark a book as a comic.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Comic flag updated successfully"),
            @ApiResponse(responseCode = "404", description = "Book not found")
    })
    @PatchMapping("/{bookId}/comic")
    @CheckBookAccess(bookIdParam = "bookId")
    @PreAuthorize("@securityUtil.canManageLibrary() or @securityUtil.isAdmin()")
    public ResponseEntity<Book> toggleComicFlag(
            @Parameter(description = "ID of the book") @PathVariable long bookId,
            @Parameter(description = "Whether the book is a comic") @RequestParam boolean comic) {
        return ResponseEntity.ok(physicalBookService.toggleComicFlag(bookId, comic));
    }
